...
import { MeterAddImageApi, MeterUpdateAudioApi } from '../services/apiService';

const ClerkAddMeterAddAudio = ({navigation}) => {
  const [isFocused, setIsFocused] = useState(false);
    const [isRecording, setIsRecording] = useState(false);
    const [buttonsEnabled, setButtonsEnabled] = useState(false);
    const {  setRecordingPath ,recordingPath, metersData,seletedJobId ,seletedJobDetails} = useUserContext() || {}; // Note the capitalization
...
const saveRecording = async () => {
  try {
    ...
    // Append meter_id as a string
    formData.append('meter_id', String(metersData?.id));
    ...
    // Make the API call
    const response = await MeterUpdateAudioApi(formData);
    console.log('Upload successful:', response.data);
    
    // Handle success
    meterImages.length > 0 && await uploadImages();

    setScreenLoading(false);
    navigation.navigate('ClerkAddMeter', { meterData: metersData, mode: 'edit' });
  } catch (error) {
    setScreenLoading(false);
...
export default ClerkAddMeterAddAudio;
