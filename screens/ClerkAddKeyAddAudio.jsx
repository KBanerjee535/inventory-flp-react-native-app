...
import { KeyAddImageApi, KeyUpdateAudioApi } from '../services/apiService';
...
const ClerkAddKeyAddAudio = ({navigation}) => {
  const [isFocused, setIsFocused] = useState(false);
  const [isRecording, setIsRecording] = useState(false);
  const [buttonsEnabled, setButtonsEnabled] = useState(false);
  const {  setRecordingPath ,recordingPath, keysData,seletedJobId } = useUserContext() || {}; // Note the capitalization
...
const handleCamera = () => {
  ...
  const handleGallery = () => {
    launchImageLibrary({mediaType: 'photo', selectionLimit: 0}, response => {
      ...
      setKeyImages(prev => [...prev, ...selectedAssets]);
    });
  };
...
const stopRecording = async () => {
  ...
  const uploadImages = async () => {
      ...
      let response = await KeyAddImageApi(fd);
...
const saveRecording = async () => {
  try {
    ...
    formData.append('key_id', String(keysData?.id));
    ...
    // Make the API call
    const response = await KeyUpdateAudioApi(formData);
    console.log('Upload successful:', response.data);
    
    // Handle success
    keyImages.length > 0 && await uploadImages();

    setScreenLoading(false);
    navigation.navigate('ClerkAddKey', { keyData: keysData, mode: 'edit' });
  } catch (error) {
    setScreenLoading(false);
          ...
          <View style={styles.Body}>
            <View style={styles.Header}>
              <TouchableOpacity
                ...
                onPress={() => navigation.navigate('ClerkAddKey', {
                      keyData: keysData,
                      mode: 'edit'
                    })}>
...
export default ClerkAddKeyAddAudio;
