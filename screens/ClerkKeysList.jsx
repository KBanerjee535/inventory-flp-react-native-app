...
import {KeyListByInventoryIdApi} from '../services/apiService';
...
const ClerkKeysList = ({navigation}) => {
  const fadeAnims = useRef(items.map(() => new Animated.Value(0))).current; // Create an array of Animated values
 const { userData, setIsLoggedIn, seletedJobId,keylist,setKeylist} = useUserContext();
  ...
  try {
    ...
    let response = await KeyListByInventoryIdApi(fd);
   
    setKeylist(response?.data?.data); 
    console.log('Key List Response:', keylist);
      ...
      <View style={styles.Body}>
        ...
        <ScrollView style={styles.container}>
          <View style={styles.List}>
            {Array.isArray(keylist) && keylist?.map((item, index) => (
              <Animated.View key={item.id} style={{opacity: fadeAnims[index]}}>
                <TouchableOpacity
                  style={styles.ListItem}
                 onPress={() => {                    
                    navigation.navigate('ClerkAddKey', {
                      keyData: item,
                      mode: 'edit'
                    });
                  }}>
                  <View style={styles.ListItemInner}>
                    <ItemIcon3 width={24} height={24} />
                    <Text style={styles.ListItemTxt}>{item.key_title}</Text>
                  </View>
                  <Arrow
            ...
            <TouchableOpacity
              ...
              onPress={() => navigation.navigate('ClerkAddKey', { mode: 'add' })}>
              <Text style={styles.NextBtnTxt}>
                {keylist?.length > 0 ? 'Add Another Key' : 'Add Your First Key'}               
              </Text>
            </TouchableOpacity>
          </View>
        </ScrollView>
